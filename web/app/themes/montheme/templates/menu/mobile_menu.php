<?php
$mobile_header_width = esc_attr(HERITAGE_get_header_footer_width());
...
<div class="header_inner lc_mobile_menu <?php echo esc_attr($mobile_header_width); ?>">
    ...
    <div class="creative_right">
        ...
        <?php if (HERITAGE_is_woocommerce_active()) {?>
            <div class="mobile_menu_icon creative_header_icon lc_icon_creative_cart heritage-minicart-icon">
        ...
        <?php if( in_array( HERITAGE_get_menu_style(), array('creative_menu','classic_menu') ) && HERITAGE_is_woocommerce_active()) { ?>
            <div class="creative_header_icon mobile_menu_icon at_login">
                <?php if (is_user_logged_in()) { ?>
                    <a href="<?php echo esc_attr(get_permalink(get_option('woocommerce_myaccount_page_id'))); ?>"
                       ...
                       title="<?php esc_attr_e('My Account', 'heritage'); ?>"><i class="fa fa-user"></i></a>
                <?php } else { ?>
                    <a href="<?php echo esc_attr(get_permalink(get_option('woocommerce_myaccount_page_id'))); ?>"
                       class="<?php echo esc_attr( HERITAGE_is_login_popup_enabled() ? 'at_to_login_popup' : '' ) ?>"
                       title="<?php esc_attr_e('Login &#47; Signup', 'heritage'); ?>"><i class="fa fa-sign-in"></i></a>
                <?php } ?>
            </div>
        <?php } ?>
    </div>
