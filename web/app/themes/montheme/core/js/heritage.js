jQuery( document ).ready( function( $ ) {
    ...
    $( window ).on( "debouncedresize", function( event ) {
        ...
        heritageHandleProductsMasonry( $ );
    } );

    imageOverText( $ );
    handlePostRating($);
    heritageSelect2( $ );
    heritageGridListSelector( $ );
    heritageItemsPerRowSelector( $ );
    heritageAddedToCartHandler( $ );
    heritageHandleLoginPopup( $ );
    heritageHandleQuickView( $ );

    handleType2ProductSliderImgHeight( $ );

    heritageHandleType2TemplateVariationChangeImage($);
    heritageHandleProductsMasonry($);
...
// Grab style sheet
heritageStylesheet = heritageStyleElement.sheet;
...
function heritageAddStylesheet ( id, cssText, context ) {
    ...
    if ( (foundIndex = heritageRulesIdIndexes.indexOf( id )) > -1 ) {
        heritageStylesheet.deleteRule( foundIndex );
        heritageRulesIndexes.splice( foundIndex, 1 );
        heritageRulesIdIndexes.splice( foundIndex, 1 );
    }
    heritageStylesheet.insertRule( cssText, heritageStylesheet.cssRules.length );
    heritageRulesIndexes.push( {
        'context' : context,
        'ruleId'  : id
    } );
    heritageRulesIdIndexes.push( id );
}
...
function handleJSStyling ( $ ) {
    ...
    $( ".at_swp_js_css" ).each( function() {
        ...
        if ($(this).parent().hasClass("use_custom_link_color")) {
            ...
            var elementCustomClass = 'at_'+ heritageGuid();
            ...
            if ($(this).hasClass("at_link_line_before")) {
                ...
                heritageAddStylesheet( elementCustomClass, cssText, 'at_link_line_before');
                cssText = '.'+elementCustomClass+':hover { color: ' + cust_hover_col +' !important; }';
                heritageAddStylesheet( elementCustomClass + '-hover', cssText, 'at_link_line_before');

                cssText = '.'+elementCustomClass+':before { background-color: ' +cust_col +' !important; }';
                heritageAddStylesheet( elementCustomClass + '-before', cssText, 'at_link_line_before');
                cssText = '.'+elementCustomClass+':hover:before { background-color: ' + cust_hover_col +' !important; }';
                heritageAddStylesheet( elementCustomClass+ '-before-hover', cssText, 'at_link_line_before');
            }
        }
    } );
...
function customPageMenuStyle($) {
    ...
    if ($('header#at_page_header').hasClass('cust_page_menu_style')) {
        ...
        if (menu_col && (menu_col != "")) {
            /*creative*/
            $('header#at_page_header').find(".hmb_line").css("background-color", menu_col);
            $('header#at_page_header').find(".at_login_wish > div > a").css("color", menu_col);
            $('header#at_page_header').find(".creative_header_icon > a").css("color", menu_col);
            $('header#at_page_header').find(".creative_header_icon > .lnr-magnifier").css("color", menu_col);
...
var runUnslider = function( $ ) {
    ...
    gallery.on( 'unslider.change', function( event, index, slide ) {
        $( '.heritage_gallery_thumbnails a.heritage_gallery_thumbnail' ).removeClass( 'active' );
        $( '.heritage_gallery_thumbnails a.heritage_gallery_thumbnail:eq(' + index + ')' ).addClass( 'active' );
    } );
    ...
    $( '.heritage_gallery_thumbnails a' ).click( function( event ) {
        event.preventDefault();
...
var heritageQuantityChanger = function( $ ) {
...
var heritageGridListSelector = function( $ ) {
...
var heritageItemsPerRowSelector = function( $ ) {
...
var heritageHandleLoginPopup = function( $ ) {
    ...
    $( '#at_login_btn' ).on( 'click', function( event ) {
        ...
        formData.push( {name : 'action', value : 'heritage_ajax_login'} );
        formData.push( {name : this.name, value : this.value} );

        $.ajax( {
            url        : heritage_login_popup.ajax_url,
            method     : 'POST',
            data       : formData,
            beforeSend : function() {
                $( '#at_login_popup_messages' ).removeClass( 'active' ).html( '' );
            ...
            success    : function( response ) {
                ...
                if ( typeof response.success !== "undefined" && response.success ) {
                    ...
                    var msg = heritage_login_popup.general_error_text;
            ...
            error      : function( a, b, c ) {
                $( '#at_login_popup_messages' ).html( heritage_login_popup.general_error_text ).addClass( 'active' );
            },
            complete   : function() {
...
var heritageHandleQuickView = function( $ ) {

    $( '.heritage_quickview_button a' ).fancybox( {
...
var heritageConfirmBox = function( opts ) {

    var defaults = {
        title            : '',
        message          : '',
        cancelButtonText : heritage.confirmCancel,
        okButtonText     : heritage.confirmOk,
        callback         : null
    };
...
var heritageAlertBox = function( opts ) {

    var defaults = {
        title        : '',
        message      : '',
        okButtonText : heritage.alertOk,
        callback     : null
    };
...
(function( proxied ) {
    window.alert = function( message ) {
        // do something here
        return heritageAlertBox( {'message' : message} );
    };
})( window.alert );
...
var handleType2ProductSliderImgHeight = function( $ ) {
    var container = $( '.heritage_template-type_2.woocommerce div.images' );
    if ( container.length ) {
...
function heritageHandleType2TemplateVariationChangeImage($) {
    $('.heritage_template-type_2 .variations_form').on('wc_variation_form', function(event){
...
function heritageHandleProductsMasonry($) {
            ...
            'success'    : function( response ) {
                ...
                if ( searchTerm == $( '#search-word', $container ).val().trim() ) {
                    ...
                    if ( typeof response.posts !== "undefined" ) {
                        ...
                        var height = 0;
                        $( '.search_results', $container ).find( '.heritage_post' ).each( function() {
                            var selfHeight = $( this ).outerHeight();
                            if ( height < selfHeight ) {
                                height = selfHeight;
                            }
                        } );
                        $( '.search_results', $container ).find( '.heritage_post' ).css( 'height', height );
                    }
                }
            },
            'complete'   : function() {
